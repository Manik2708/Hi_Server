...
export class ConfessionsController{
    constructor(
        private readonly confessionServices: ConfessionServices
    ){}

    @Post('send-confession')
    async sendConfessionToCrush(@Req() req: express.Request, @Res() res: express.Response){
        await this.confessionServices.sendConfessionToUser(req, res);
    }

    @Post('reject-confession')
    async rejectConfession(@Req() req: express.Request, @Res() res: express.Response){
