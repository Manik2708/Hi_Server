...
export const requestUnreadRecievedConfessionsFxn=async(client:RedisClientType, lastConfessionId:string, userId:string ):Promise<Array<UnreadRecievedConfessions>>=>{
    const list:Array<UnreadRecievedConfessions>=[]
    const tryRetrivingFromRedis=await client.get(RedisNames.LastRecievedConfession+userId)
