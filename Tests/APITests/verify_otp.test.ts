...
describe('Verify OTP API test', ()=>{
    ...
    it('No otp/token sent', async()=>{
        const response=await request(serverInstance.app).post('/verify-email').send({}).expect(400)
    ...
    it('No otp sent',async () => {
        ...
        const response=await request(serverInstance.app).post('/verify-email').send(mockRequest).expect(400)
        expect(response.body.msg).toBe("OTP or token is not provided")
    })
    ...
    it('Right OTP is sent', async()=>{
       ...
       const response=await request(serverInstance.app).post('/verify-email').send(mockRequest).expect(200)
