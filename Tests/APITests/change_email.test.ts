...
describe('Change Email API test', ()=>{
    ...
    it('No password sent',async () => {
        ...
        const response=await request(serverInstance.app).post('/change-email').send(mockRequest).expect(400)
    ...
    it('Wrong password sent',async () => {
        ...
        vi.spyOn(bcrypt, 'compare').mockImplementationOnce(async(...args: any):Promise<boolean>=>{
            return false;
        })
        const response=await request(serverInstance.app).post('/change-email').send(mockRequest).expect(400)
    ...
    it('Right password sent',async () => {
        ...
        const response=await request(serverInstance.app).post('/change-email').send(mockRequest).expect(200)
