...
describe('API test for Creating New Account', ()=>{
    ...
    it('If user with same email address try to create account', async()=>{
        ...
        const response=await request(serverInstance.app).post('/create-account-without-verification').send(mockRequest).expect(400)
        expect(response.body.msg).toBe('User with same Email or Phone exists. Please Login!')
    })
    it('Username is same', async()=>{
        ...
        const response=await request(serverInstance.app).post('/create-account-without-verification').send(mockRequest).expect(400)
        expect(response.body.msg).toBe("Username not available, kindly pick a new one")
    })
    it('User send correct request', async()=>{
        ...
        const response=await request(serverInstance.app).post('/create-account-without-verification').send(mockRequest).expect(200)
